...
def main():
    ...
    initialDistributionList = ZonedDistribution(distribution_callback = CosineSimilarityDistribution, metric_callback = NumericMetrics.L2Norm, alpha=0.75) 
    centroid_list = []
    ...
    initialDistributionList.build(centroid, train_embeddings_tmp)
    
    print( 'delta_low', initialDistributionList.attrs['delta_low'], \
             'delta_high', initialDistributionList.attrs['delta_high'])
    ...
    for lidx, train_label in enumerate(train_label_list):
        ...
        for _data in train_embeddings_tmp:
            ...
            if tmp_dis < initialDistributionList.attrs['delta_high']:
                # hit
                tp += 1
                isHit = True
            else:
    ...
    for lidx, test_label in enumerate(test_label_list):
        ...
        for _data in train_embeddings_tmp:
            tmp_dis = getDistance(_data, centroid)                          
            if tmp_dis < initialDistributionList.attrs['delta_high'] and tmp_dis > initialDistributionList.attrs['delta_low']:
                # false hit
                fp += 1     
            else:
                tn += 1
