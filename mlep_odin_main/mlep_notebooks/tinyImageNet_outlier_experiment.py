...
def main():
    ...
    # initialDistributionList = [ZonedDistribution(distribution_callback = CosineSimilarityDistribution, metric_callback = TextMetrics.inverted_cosine_similarity) for i in range(len(train_label_list))]
    initialDistributionList = [ZonedDistribution(distribution_callback = CosineSimilarityDistribution, metric_callback = NumericMetrics.L2Norm, alpha=0.6) for i in range(len(train_label_list))]
    centroid_list = []
    for lidx, train_label in enumerate(train_label_list):
        ...
        initialDistributionList[lidx].build(centroid, train_embeddings_tmp)
        # print('label', lidx, 'delta_low', initialDistributionList[lidx].attrs['delta_low'], \
        #         'delta_high', initialDistributionList[lidx].attrs['delta_high'])
        # get distribution by adding them individually
        # for _data in train_embeddings_tmp:
        #     initialDistributionList[lidx].update(getDistance(_data, centroid))
    ...
    for lidx, train_label in enumerate(train_label_list):
        ...
        isHit=False
        for _data in train_embeddings_tmp:
            tmp_dis = getDistance(_data, centroid_list[lidx])
            if tmp_dis < initialDistributionList[lidx].attrs['delta_high']:
                # hit
                tp += 1
                isHit = True
            else:
    ...
    for lidx, test_label in enumerate(test_label_list):
        ...
        isHit=False
        for _data in train_embeddings_tmp:
            for cidx, centroid in enumerate(centroid_list):
                tmp_dis = getDistance(_data, centroid_list[cidx])                          
                if tmp_dis < initialDistributionList[cidx].attrs['delta_high']:
                    # false hit
                    fp += 1     
                    isHit = True 
                    break
    ...
    secondary_distribution = CosineSimilarityDistribution.CosineSimilarityDistribution(nBins)
    processLength = 0
    #genCount = 0
    #axv = []
