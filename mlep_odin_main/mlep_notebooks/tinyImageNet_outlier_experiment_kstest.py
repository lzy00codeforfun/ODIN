...
def make_prediction(cd, query_list, isFlag):
    ...
    for x in query_list:
        ...
        # preds = cd.predict(x)
        preds = cd.predict(np.expand_dims(x, axis=0), drift_type='batch', return_p_val=True, return_distance=True)
...
def main():
    ...
    p_val = .05
    drift_detector = KSDrift(train_embeddings_tmp, p_val=p_val)
