...
def scatter_plot(latent_representations, labels):
    ...
    plt.grid()
    plt.show()
    plt.savefig('fig_folder/imagenet_{}_{}.png'.format(model, train_num+test_num), format='png')
...
def main():
    ...
    for lidx, train_label in enumerate(train_label_list):
        ...
        labels = [lidx for i in range(train_embeddings_tmp.shape[0])]
        embeddings_list.append(train_embeddings_tmp)
    ...
    for lidx, test_label in enumerate(test_label_list):
        ...
        train_embeddings_tmp = pkl.load(open(f_path, 'rb'))
        train_embeddings_tmp, _ = transform_pkl2npy(train_embeddings_tmp)
        labels = [lidx+train_num for i in range(train_embeddings_tmp.shape[0])]
        embeddings_list.append(train_embeddings_tmp)
        labels_list.append(labels)
    
    print('embeddings', len(embeddings_list), embeddings_list[0].shape)
    print('labels', len(labels_list), len(labels_list[0]))
    embeddings_list = np.concatenate(embeddings_list, axis=0)
    labels_list = np.concatenate(labels_list, axis=0)
    print('after----')
    print(len(embeddings_list), embeddings_list[0].shape)
    print(len(labels_list))             
    
    embed_tsne = tsne_trans(embeddings_list)
    print(embed_tsne.shape)
    scatter_plot(embed_tsne, labels_list)
